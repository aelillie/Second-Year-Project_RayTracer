...
let main argv =
    ...
    let render toScreen =
        (*******Light******)
        let light = mkLight (mkPoint 0.0 2.0 0.0) (fromColor Color.White) 1.0 in
        let light2 = mkLight (mkPoint 0.0 2.0 2.0) (fromColor Color.White) 1.0 in
        let ambientLight = mkAmbientLight (fromColor Color.White) 0.3 in
        (*******Camera******)
        let camera = mkCamera (mkPoint 0.0 0.75 0.75) (mkPoint 0.0 0.0 0.0) (mkVector 0.0 1.0 0.0) 1.0 2.0 2.0 1500 1500 in
        (*******Shapes******)
        let stopWatch = System.Diagnostics.Stopwatch.StartNew()
        let tm = mkPLY "C:\\Program Files\\bunny.ply" false
        let tm' = mkShape tm (Texture.mkMatTexture ((Material.mkMaterial (Colour.fromColor Color.Gray)) 0.0))
        let tm'' = transform tm' (scale 2.0 2.0 2.0)
