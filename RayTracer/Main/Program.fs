...
let main argv =
    ...
    let render toScreen =
        ...
        (*******Shapes******)
        let ply = mkPLY "../../../ply/porsche.ply" false
        let porsche = mkShape ply (mkMatTexture (mkMaterial (fromColor Color.Gray) 0.0))

          
        (*******Scene******)

        let scene = mkScene [porsche] [light] ambientLight camera 3
        if toScreen then
