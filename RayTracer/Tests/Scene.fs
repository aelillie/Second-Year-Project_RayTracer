...
let renderToFile (S(shapes, lights, ambi, cam, n)) (filename:string) =
    ...
    let rec castRay (ray:Ray) n = 
        ...
        let lightIntensity (ray:Ray) (s:Shape) (l:Light) nV =
         
         match hit ray s with
         |None    -> let i = (Light.calculateI nV (Ray.getD ray) (Light.getLightI l))
                     i

         |Some(t',_,_) -> let o = Camera.getPoint cam
                          let tlight = Point.distance (Ray.getP ray) o |> Vector.magnitude  
                          if t' > tlight 
                          then 
                            let i =(Light.calculateI nV (Ray.getD ray) (Light.getLightI l))
                            i
                          else
                            0.0
        ...
        match intersects with 
         ...
         | _  -> let (t,nV,c) = List.minBy (fun (t,_,_) -> t ) intersects
                 ...
                 let i' = List.fold (fun acc x -> x + acc) 0.0 il
