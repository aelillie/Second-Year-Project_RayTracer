...
open TransformedShape
  ...
  (* Input the path (local or absolute) where you want your files to be stored *)
      ...
      let renderSphere toScreen =
        ...
        let blueSphere = mkSphere (mkPoint 0.5 0.0 0.0) 1.0 (Material.mkMaterial (Colour.fromColor Color.Blue) 0.2)
        let redSphere = mkSphere (mkPoint -0.5 0.0 0.0) 1.0 (Material.mkMaterial (Colour.fromColor Color.Red) 0.2)
        let blueRedSC = mkSolidCylinder (mkPoint 0.5 0.0 0.0) 1.0 2.0
                            (Material.mkMaterial (Colour.fromColor Color.Blue) 0.2)
                            (Material.mkMaterial (Colour.fromColor Color.Red) 0.2)
                            (Material.mkMaterial (Colour.fromColor Color.Red) 0.2)
        let greenYellowSC = mkSolidCylinder (mkPoint -0.5 0.0 0.0) 1.0 2.0
        ...
        let blueRedSphere = intersection (transform blueSphere (translate -0.5 0.0 0.0)) (transform redSphere (translate 0.5 0.0 0.0))                    
