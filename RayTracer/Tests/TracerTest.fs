...
open Implicit
  ...
  (* Input the path (local or absolute) where you want your files to be stored *)
      ...
      let renderSphere toScreen =
        ...
        let plane = mkPlane(mkPoint 0.0 -10.0 0.0) (mkVector 0.0 -1.0 -1.5) (Material.mkMaterial (Colour.mkColour 0.35 0.24 0.67) 0.0)
        ...
        let implicitSphere = mkShape (mkImplicit "x^2+y^2+z^2+-1r^2" ("r",1.0)) (Material.mkMaterial (Colour.fromColor Color.Purple) 0.05)
        ...
        let implicitSphere4 = mkShape (mkImplicit "x^2+y^2+z^2+-1r^2" ("r",1.0)) (Material.mkMaterial (Colour.fromColor Color.Blue) 0.05)
        let implicitPlane = mkShape (mkImplicit "2*x+2*y+2*z+d" ("d",1.0)) (Material.mkMaterial (Colour.fromColor Color.Yellow) 0.2)
        ...
        let timplic1 = transform implicitSphere (mergeTransformations [translate 1.0 1.0 0.0])
        ...
        let timplic4 = transform implicitSphere4 (mergeTransformations [translate 1.0 -1.5 0.0])
        ...
        let scene = Scene.mkScene [timplic1;timplic2;timplic3;timplic4;plane] [light] ambientLight camera 2
    

        if toScreen then
          doRender scene None
        else
          doRender scene (Some ("transform4.png"))
